...
class LearningContextBuilder:
    ...
    def build_live_context(
        ...
        # Group-4 Step-1 scaffold hints:
        # Add only short/high-confidence safe meta-signals. Neutral bundles should stay quiet.
        ...
        if safe_float(g4_emotional.get("confidence", 0.0), 0.0) >= 0.7:
            candidates.append(
                {
                    "type": "group4_signal",
                    "priority": 0.64,
                    "topic": "emotional_graph",
                    "text": "Group-4 emotional graph hint: duygusal sureklilik sinyali var; tonu dengeli ve net tut.",
                    "source": "group4_bundle",
                    "confidence": 0.74,
                    "ts": now_utc.isoformat().replace("+00:00", "Z"),
                }
            )
        if safe_float(g4_time.get("confidence", 0.0), 0.0) >= 0.7:
            candidates.append(
                {
                    "type": "group4_signal",
                    "priority": 0.61,
                    "topic": "time_ecology",
                    "text": "Group-4 time ecology hint: ritmi sade tut, gereksiz hizlanmadan ilerle.",
                    "source": "group4_bundle",
                    "confidence": 0.72,
                    "ts": now_utc.isoformat().replace("+00:00", "Z"),
                }
            )
        if safe_float(g4_cultural.get("confidence", 0.0), 0.0) >= 0.7:
            candidates.append(
                {
                    "type": "group4_signal",
                    "priority": 0.63,
                    "topic": "cultural_epistemic",
                    "text": "Group-4 cultural/epistemic hint: kesin yargi yerine acik ve saygili ifade sec.",
                    "source": "group4_bundle",
                    "confidence": 0.74,
                    "ts": now_utc.isoformat().replace("+00:00", "Z"),
                }
            )
        if safe_float(g4_reflection.get("confidence", 0.0), 0.0) >= 0.7:
            candidates.append(
                {
                    "type": "group4_signal",
                    "priority": 0.62,
                    "topic": "reflection",
                    "text": "Group-4 reflection hint: gerekirse tek mini netlestirme adimiyla devam et.",
                    "source": "group4_bundle",
                    "confidence": 0.73,
                    "ts": now_utc.isoformat().replace("+00:00", "Z"),
                }
