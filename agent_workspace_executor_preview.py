...
MODULE = "agent_workspace_executor"
CAPABILITIES = ["workspace_change_preview", "clone_sandbox_relation_preview", "workspace_transition_preview", "visibility_mapping"]
INTEGRATION_POINTS = ["39.2_agent_workspace_runtime", "37.5_workspace_agent", "40.2_agent_task_executor"]
...
def agent_workspace_executor_status() -> Dict[str, Any]:
    return {"layer": LAYER, "series": SERIES, "module": MODULE, "status": "agent_workspace_executor_preview_ready", "operation_mode": "read_only_preview_only", "capabilities": CAPABILITIES, "integration_points": INTEGRATION_POINTS, "read_only": True, "safety": _safety()}


def agent_workspace_executor_capabilities() -> Dict[str, Any]:
...
def agent_workspace_executor_preview(payload: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "layer": LAYER, "series": SERIES, "module": MODULE, "status": "preview_ready",
        "input_summary": _input_summary(payload),
        "detected_execution_type": payload.get("execution_type") or "workspace_execution_planning",
        "detected_target_system": payload.get("target_system") or "workspace_preview_only",
        "recommended_execution_model": "read_only_workspace_execution_plan",
        "workspace_changes": ["file_change_candidates_preview", "state_transition_preview"],
        "clone_sandbox_relation": "no_clone_or_workspace_mutation",
        "workspace_state_transitions": ["current", "planned", "verification_ready"],
        "visibility": ["visible_task_surface", "hidden_technical_state"],
        "required_confirmations": ["before_workspace_execution", "before_file_mutation"],
        "verification_gates": ["workspace_boundary_gate", "file_mutation_gate", "visibility_gate"],
        "rollback_notes": ["workspace_execution_requires_restore_point"],
        "recovery_notes": ["safe_pause_if_workspace_state_unclear"],
