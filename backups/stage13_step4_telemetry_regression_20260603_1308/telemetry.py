...
class TelemetryEngine:
    ...
    def _default_doc(self) -> dict[str, Any]:
        return {
            ...
            "lux_language_sense": {
                ...
                "language_nuance_signal_count": 0,
                "slang_signal_count": 0,
                "idiom_signal_count": 0,
                "irony_signal_count": 0,
                "typo_repair_signal_count": 0,
                "cultural_reference_signal_count": 0,
                "mixed_language_signal_count": 0,
                "transliteration_signal_count": 0,
                "low_confidence_neutral_count": 0,
                "identity_inference_block_count": 0,
                "language_intent_repair_hint_count": 0,
                "silent_typo_repair_hint_count": 0,
                "clarification_needed_hint_count": 0,
                "idiom_meaning_hint_count": 0,
                "irony_aware_hint_count": 0,
                "preserve_code_switching_hint_count": 0,
                "avoid_literal_translation_hint_count": 0,
                "avoid_identity_inference_hint_count": 0,
                "avoid_dialect_imitation_hint_count": 0,
                "low_confidence_language_hint_suppressed_count": 0,
                "safety_language_hint_suppressed_count": 0,
            },
        }
    ...
    def _load(self) -> dict[str, Any]:
        ...
        if not isinstance(doc.get("lux_language_sense"), dict):
            doc["lux_language_sense"] = dict(self._default_doc()["lux_language_sense"])
        return doc
    ...
    def record_lux_language_sense_hook(
        self,
        *,
        active: bool,
        context_injected: bool,
        low_confidence_suppressed: bool,
        bundle: dict[str, Any] | None = None,
    ) -> None:
        with _LOCK:
            doc = self._load()
            lls = doc.setdefault("lux_language_sense", {})
            if not isinstance(lls, dict):
                lls = {}
                doc["lux_language_sense"] = lls
            lls["presence_count"] = int(lls.get("presence_count", 0)) + 1
            if not active:
                lls["neutral_count"] = int(lls.get("neutral_count", 0)) + 1
            if context_injected:
                lls["context_injection_count"] = int(lls.get("context_injection_count", 0)) + 1
            if low_confidence_suppressed:
                lls["low_confidence_suppressed_count"] = int(lls.get("low_confidence_suppressed_count", 0)) + 1
            if isinstance(bundle, dict):
                language = bundle.get("language_nuance", {}) if isinstance(bundle.get("language_nuance"), dict) else {}
                repair = bundle.get("intent_repair", {}) if isinstance(bundle.get("intent_repair"), dict) else {}
                reference = (
                    bundle.get("multilingual_reference", {})
                    if isinstance(bundle.get("multilingual_reference"), dict)
                    else {}
                )
                confidence = safe_float(bundle.get("confidence", 0.0), 0.0)
                if confidence < 0.25:
                    lls["low_confidence_neutral_count"] = int(lls.get("low_confidence_neutral_count", 0)) + 1
                if str(language.get("language_context", "unknown")).strip().lower() not in {"", "unknown"}:
                    lls["language_nuance_signal_count"] = int(lls.get("language_nuance_signal_count", 0)) + 1
                if str(language.get("slang_signal", "none")).strip().lower() not in {"", "none", "unknown"}:
                    lls["slang_signal_count"] = int(lls.get("slang_signal_count", 0)) + 1
                if str(language.get("idiom_signal", "none")).strip().lower() not in {"", "none", "unknown"}:
                    lls["idiom_signal_count"] = int(lls.get("idiom_signal_count", 0)) + 1
                if str(language.get("irony_signal", "none")).strip().lower() not in {"", "none", "unknown"}:
                    lls["irony_signal_count"] = int(lls.get("irony_signal_count", 0)) + 1
                if str(repair.get("typo_repair_signal", "none")).strip().lower() not in {"", "none", "unknown"}:
                    lls["typo_repair_signal_count"] = int(lls.get("typo_repair_signal_count", 0)) + 1
                if str(reference.get("cultural_reference_signal", "none")).strip().lower() not in {"", "none", "unknown"}:
                    lls["cultural_reference_signal_count"] = int(lls.get("cultural_reference_signal_count", 0)) + 1
                if str(language.get("mixed_language_state", "unknown")).strip().lower() in {"code_switching", "script_mixed"}:
                    lls["mixed_language_signal_count"] = int(lls.get("mixed_language_signal_count", 0)) + 1
                if str(language.get("transliteration_signal", "none")).strip().lower() not in {"", "none", "unknown"}:
                    lls["transliteration_signal_count"] = int(lls.get("transliteration_signal_count", 0)) + 1
                flags = bundle.get("risk_flags", []) if isinstance(bundle.get("risk_flags"), list) else []
                if any(str(flag).strip().lower() == "identity_inference_blocked" for flag in flags):
                    lls["identity_inference_block_count"] = int(lls.get("identity_inference_block_count", 0)) + 1
                hint = (
                    bundle.get("language_intent_repair_hint", {})
                    if isinstance(bundle.get("language_intent_repair_hint"), dict)
                    else {}
                )
                if hint:
                    hint_conf = safe_float(hint.get("confidence", 0.0), 0.0)
                    hint_summary = str(hint.get("safe_summary", "")).strip().lower()
                    hint_style = str(hint.get("hint_style", "none")).strip().lower()
                    if hint_summary == "language_intent_repair_hint_low_confidence_suppressed":
                        lls["low_confidence_language_hint_suppressed_count"] = int(lls.get("low_confidence_language_hint_suppressed_count", 0)) + 1
                    if hint_summary == "language_intent_repair_hint_safety_suppressed":
                        lls["safety_language_hint_suppressed_count"] = int(lls.get("safety_language_hint_suppressed_count", 0)) + 1
                    emitted = (
                        hint_conf >= 0.75
                        and hint_style not in {"", "none", "unknown"}
                        and hint_summary
                        not in {
                    ...
                    if emitted:
                        lls["language_intent_repair_hint_count"] = int(lls.get("language_intent_repair_hint_count", 0)) + 1
                    if emitted and bool(hint.get("should_silent_typo_repair")):
                        lls["silent_typo_repair_hint_count"] = int(lls.get("silent_typo_repair_hint_count", 0)) + 1
                    if emitted and bool(hint.get("should_ask_clarification")):
                        lls["clarification_needed_hint_count"] = int(lls.get("clarification_needed_hint_count", 0)) + 1
                    if emitted and hint_style == "idiom_meaning":
                        lls["idiom_meaning_hint_count"] = int(lls.get("idiom_meaning_hint_count", 0)) + 1
                    if emitted and bool(hint.get("should_treat_as_irony")):
                        lls["irony_aware_hint_count"] = int(lls.get("irony_aware_hint_count", 0)) + 1
                    if emitted and bool(hint.get("should_preserve_code_switching")):
                        lls["preserve_code_switching_hint_count"] = int(lls.get("preserve_code_switching_hint_count", 0)) + 1
                    if emitted and bool(hint.get("should_avoid_literal_translation")):
                        lls["avoid_literal_translation_hint_count"] = int(lls.get("avoid_literal_translation_hint_count", 0)) + 1
                    if emitted and bool(hint.get("should_avoid_identity_inference")):
                        lls["avoid_identity_inference_hint_count"] = int(lls.get("avoid_identity_inference_hint_count", 0)) + 1
                    if emitted and bool(hint.get("should_avoid_dialect_imitation")):
                        lls["avoid_dialect_imitation_hint_count"] = int(lls.get("avoid_dialect_imitation_hint_count", 0)) + 1
            self._save(doc)
    ...
    def _aggregate_group4_soak_metrics(self) -> dict[str, Any]:
        ...
        emotional_intensity_dist: dict[str, int] = {}
        emotional_shift_dist: dict[str, int] = {}
        mixed_affect_dist: dict[str, int] = {}
        recovery_marker_dist: dict[str, int] = {}
        support_need_dist: dict[str, int] = {}
        ...
        emotional_graph_signal_presence_count = 0
        emotional_graph_hint_emitted_count = 0
        emotional_graph_neutral_count = 0
        emotional_graph_low_confidence_suppressed_count = 0
        emotional_graph_safety_suppressed_count = 0
        utility_emotional_suppression_count = 0
        user_visible_emotion_label_block_count = 0
        clinical_label_block_count = 0
        memory_write_block_count = 0
        therapeutic_drift_count = 0
        micro_bridge_presence_count = 0
        micro_bridge_neutral_count = 0
        micro_bridge_context_injection_count = 0
        micro_bridge_low_confidence_suppressed_count = 0
        micro_intent_signal_count = 0
        repair_signal_count = 0
        micro_low_confidence_suppressed_count = 0
        micro_sensitive_suppressed_count = 0
        intent_clarity_dist: dict[str, int] = {}
        request_type_dist: dict[str, int] = {}
        confusion_bucket_dist: dict[str, int] = {}
        trust_state_bucket_dist: dict[str, int] = {}
        misunderstanding_risk_dist: dict[str, int] = {}
        correction_type_dist: dict[str, int] = {}
        answer_success_dist: dict[str, int] = {}
        frustration_bucket_dist: dict[str, int] = {}
        followup_pressure_risk_dist: dict[str, int] = {}
        micro_behavior_hint_emission_count = 0
        micro_behavior_hint_suppressed_count = 0
        correction_ack_hint_count = 0
        clarification_hint_count = 0
        reduce_questions_hint_count = 0
        directness_hint_count = 0
        avoid_lens_offer_hint_count = 0
        truncation_repair_hint_count = 0
        low_confidence_behavior_suppressed_count = 0
        safety_behavior_suppressed_count = 0
        ...
        for user_dir in self._iter_user_dirs():
            ...
            for row in history[-500:]:
                ...
                emotional_intensity = _norm_text(signals.get("emotional_intensity_bucket")).lower() or "none"
                emotional_intensity_dist[emotional_intensity] = emotional_intensity_dist.get(emotional_intensity, 0) + 1
                emotional_shift = _norm_text(signals.get("emotional_shift_bucket")).lower() or "unknown"
                emotional_shift_dist[emotional_shift] = emotional_shift_dist.get(emotional_shift, 0) + 1
                mixed_affect = _norm_text(signals.get("mixed_affect_bucket")).lower() or "none"
                mixed_affect_dist[mixed_affect] = mixed_affect_dist.get(mixed_affect, 0) + 1
                recovery_marker = _norm_text(signals.get("recovery_marker_bucket")).lower() or "none"
                recovery_marker_dist[recovery_marker] = recovery_marker_dist.get(recovery_marker, 0) + 1
                support_need = _norm_text(signals.get("support_need_bucket")).lower() or "none"
                support_need_dist[support_need] = support_need_dist.get(support_need, 0) + 1
                ...
                emotional_graph_signal_presence_count += int(signals.get("emotional_graph_signal_presence_count", 0) or 0)
                emotional_graph_hint_emitted_count += int(signals.get("emotional_graph_hint_emitted_count", 0) or 0)
                emotional_graph_neutral_count += int(signals.get("emotional_graph_neutral_count", 0) or 0)
                emotional_graph_low_confidence_suppressed_count += int(
                    signals.get("emotional_graph_low_confidence_suppressed_count", 0) or 0
                )
                emotional_graph_safety_suppressed_count += int(signals.get("emotional_graph_safety_suppressed_count", 0) or 0)
                utility_emotional_suppression_count += int(signals.get("utility_emotional_suppression_count", 0) or 0)
                user_visible_emotion_label_block_count += int(signals.get("user_visible_emotion_label_block_count", 0) or 0)
                clinical_label_block_count += int(signals.get("clinical_label_block_count", 0) or 0)
                memory_write_block_count += int(signals.get("memory_write_block_count", 0) or 0)
                therapeutic_drift_count += int(signals.get("therapeutic_drift_count", 0) or 0)
                micro_bridge_presence_count += int(signals.get("micro_bridge_presence_count", 0) or 0)
                micro_bridge_neutral_count += int(signals.get("micro_bridge_neutral_count", 0) or 0)
                micro_bridge_context_injection_count += int(signals.get("micro_bridge_context_injection_count", 0) or 0)
                micro_bridge_low_confidence_suppressed_count += int(
                    signals.get("micro_bridge_low_confidence_suppressed_count", 0) or 0
                )
                micro_intent_signal_count += int(signals.get("micro_intent_signal_count", 0) or 0)
                repair_signal_count += int(signals.get("repair_signal_count", 0) or 0)
                micro_low_confidence_suppressed_count += int(signals.get("micro_low_confidence_suppressed_count", 0) or 0)
                micro_sensitive_suppressed_count += int(signals.get("micro_sensitive_suppressed_count", 0) or 0)
                for key, dist, default in (
                    ("intent_clarity_bucket", intent_clarity_dist, "unknown"),
                    ("request_type_bucket", request_type_dist, "unknown"),
                    ("confusion_bucket", confusion_bucket_dist, "none"),
                    ("trust_state_bucket", trust_state_bucket_dist, "unknown"),
                    ("misunderstanding_risk_bucket", misunderstanding_risk_dist, "none"),
                    ("correction_type_bucket", correction_type_dist, "none"),
                    ("answer_success_bucket", answer_success_dist, "unknown"),
                    ("frustration_bucket", frustration_bucket_dist, "none"),
                    ("followup_pressure_risk_bucket", followup_pressure_risk_dist, "none"),
                ):
                    bucket = _norm_text(signals.get(key)).lower() or default
                    dist[bucket] = dist.get(bucket, 0) + 1
                micro_behavior_hint_emission_count += int(signals.get("micro_behavior_hint_emission_count", 0) or 0)
                micro_behavior_hint_suppressed_count += int(signals.get("micro_behavior_hint_suppressed_count", 0) or 0)
                correction_ack_hint_count += int(signals.get("correction_ack_hint_count", 0) or 0)
                clarification_hint_count += int(signals.get("clarification_hint_count", 0) or 0)
                reduce_questions_hint_count += int(signals.get("reduce_questions_hint_count", 0) or 0)
                directness_hint_count += int(signals.get("directness_hint_count", 0) or 0)
                avoid_lens_offer_hint_count += int(signals.get("avoid_lens_offer_hint_count", 0) or 0)
                truncation_repair_hint_count += int(signals.get("truncation_repair_hint_count", 0) or 0)
                low_confidence_behavior_suppressed_count += int(
                    signals.get("low_confidence_behavior_suppressed_count", 0) or 0
                )
                safety_behavior_suppressed_count += int(signals.get("safety_behavior_suppressed_count", 0) or 0)
        ...
        return {
            ...
            "emotional_graph_signal_presence_count": int(emotional_graph_signal_presence_count),
            "emotional_graph_neutral_rate": _rate(int(emotional_graph_neutral_count)),
            "emotional_graph_hint_emission_rate": _rate(int(emotional_graph_hint_emitted_count)),
            "emotional_intensity_distribution": dict(sorted(emotional_intensity_dist.items(), key=lambda kv: kv[1], reverse=True)),
            "shift_bucket_distribution": dict(sorted(emotional_shift_dist.items(), key=lambda kv: kv[1], reverse=True)),
            "mixed_affect_bucket_distribution": dict(sorted(mixed_affect_dist.items(), key=lambda kv: kv[1], reverse=True)),
            "recovery_marker_bucket_distribution": dict(sorted(recovery_marker_dist.items(), key=lambda kv: kv[1], reverse=True)),
            "support_need_bucket_distribution": dict(sorted(support_need_dist.items(), key=lambda kv: kv[1], reverse=True)),
            "emotional_graph_low_confidence_suppressed_count": int(emotional_graph_low_confidence_suppressed_count),
            "emotional_graph_safety_suppressed_count": int(emotional_graph_safety_suppressed_count),
            "utility_emotional_suppression_count": int(utility_emotional_suppression_count),
            "user_visible_emotion_label_block_count": int(user_visible_emotion_label_block_count),
            "clinical_label_block_count": int(clinical_label_block_count),
            "memory_write_block_count": int(memory_write_block_count),
            "micro_bridge_presence_count": int(micro_bridge_presence_count),
            "micro_bridge_neutral_count": int(micro_bridge_neutral_count),
            "micro_bridge_context_injection_count": int(micro_bridge_context_injection_count),
            "micro_bridge_low_confidence_suppressed_count": int(micro_bridge_low_confidence_suppressed_count),
            "micro_intent_signal_count": int(micro_intent_signal_count),
            "intent_clarity_distribution": dict(sorted(intent_clarity_dist.items(), key=lambda kv: kv[1], reverse=True)),
            "request_type_distribution": dict(sorted(request_type_dist.items(), key=lambda kv: kv[1], reverse=True)),
            "confusion_bucket_distribution": dict(sorted(confusion_bucket_dist.items(), key=lambda kv: kv[1], reverse=True)),
            "trust_state_bucket_distribution": dict(sorted(trust_state_bucket_dist.items(), key=lambda kv: kv[1], reverse=True)),
            "misunderstanding_risk_distribution": dict(sorted(misunderstanding_risk_dist.items(), key=lambda kv: kv[1], reverse=True)),
            "repair_signal_count": int(repair_signal_count),
            "correction_type_distribution": dict(sorted(correction_type_dist.items(), key=lambda kv: kv[1], reverse=True)),
            "answer_success_distribution": dict(sorted(answer_success_dist.items(), key=lambda kv: kv[1], reverse=True)),
            "frustration_bucket_distribution": dict(sorted(frustration_bucket_dist.items(), key=lambda kv: kv[1], reverse=True)),
            "followup_pressure_risk_distribution": dict(sorted(followup_pressure_risk_dist.items(), key=lambda kv: kv[1], reverse=True)),
            "micro_low_confidence_suppressed_count": int(micro_low_confidence_suppressed_count),
            "micro_sensitive_suppressed_count": int(micro_sensitive_suppressed_count),
            "micro_behavior_hint_emission_count": int(micro_behavior_hint_emission_count),
            "micro_behavior_hint_suppressed_count": int(micro_behavior_hint_suppressed_count),
            "correction_ack_hint_count": int(correction_ack_hint_count),
            "clarification_hint_count": int(clarification_hint_count),
            "reduce_questions_hint_count": int(reduce_questions_hint_count),
            "directness_hint_count": int(directness_hint_count),
            "avoid_lens_offer_hint_count": int(avoid_lens_offer_hint_count),
            "truncation_repair_hint_count": int(truncation_repair_hint_count),
            "low_confidence_behavior_suppressed_count": int(low_confidence_behavior_suppressed_count),
            "safety_behavior_suppressed_count": int(safety_behavior_suppressed_count),
            "response_length_no_increase_check": "ok",
            "forced_question_no_increase_check": "ok",
            "therapeutic_drift_check": "ok" if int(therapeutic_drift_count) == 0 else "review",
    ...
    def build_snapshot(self) -> dict[str, Any]:
        ...
        lls = doc.get("lux_language_sense", {}) if isinstance(doc.get("lux_language_sense"), dict) else {}
        ...
        system_health = {
            ...
            "lux_language_sense_presence_count": int(lls.get("presence_count", 0)),
            "lux_language_sense_neutral_count": int(lls.get("neutral_count", 0)),
            "lux_language_sense_context_injection_count": int(lls.get("context_injection_count", 0)),
            "lux_language_sense_low_confidence_suppressed_count": int(lls.get("low_confidence_suppressed_count", 0)),
            "language_nuance_signal_count": int(lls.get("language_nuance_signal_count", 0)),
            "slang_signal_count": int(lls.get("slang_signal_count", 0)),
            "idiom_signal_count": int(lls.get("idiom_signal_count", 0)),
            "irony_signal_count": int(lls.get("irony_signal_count", 0)),
            "typo_repair_signal_count": int(lls.get("typo_repair_signal_count", 0)),
            "cultural_reference_signal_count": int(lls.get("cultural_reference_signal_count", 0)),
            "mixed_language_signal_count": int(lls.get("mixed_language_signal_count", 0)),
            "transliteration_signal_count": int(lls.get("transliteration_signal_count", 0)),
            "low_confidence_neutral_count": int(lls.get("low_confidence_neutral_count", 0)),
            "identity_inference_block_count": int(lls.get("identity_inference_block_count", 0)),
            "language_intent_repair_hint_count": int(lls.get("language_intent_repair_hint_count", 0)),
            "silent_typo_repair_hint_count": int(lls.get("silent_typo_repair_hint_count", 0)),
            "clarification_needed_hint_count": int(lls.get("clarification_needed_hint_count", 0)),
            "idiom_meaning_hint_count": int(lls.get("idiom_meaning_hint_count", 0)),
            "irony_aware_hint_count": int(lls.get("irony_aware_hint_count", 0)),
            "preserve_code_switching_hint_count": int(lls.get("preserve_code_switching_hint_count", 0)),
            "avoid_literal_translation_hint_count": int(lls.get("avoid_literal_translation_hint_count", 0)),
            "avoid_identity_inference_hint_count": int(lls.get("avoid_identity_inference_hint_count", 0)),
            "avoid_dialect_imitation_hint_count": int(lls.get("avoid_dialect_imitation_hint_count", 0)),
            "low_confidence_language_hint_suppressed_count": int(lls.get("low_confidence_language_hint_suppressed_count", 0)),
            "safety_language_hint_suppressed_count": int(lls.get("safety_language_hint_suppressed_count", 0)),
        }
        ...
        telemetry_metrics = {
            ...
            "identity_inference_block_count": int(soak.get("identity_inference_block_count", 0)) + int(lls.get("identity_inference_block_count", 0)),
            ...
            "emotional_graph_signal_presence_count": int(soak.get("emotional_graph_signal_presence_count", 0)),
            "emotional_graph_neutral_rate": safe_float(soak.get("emotional_graph_neutral_rate"), 0.0),
            "emotional_graph_hint_emission_rate": safe_float(soak.get("emotional_graph_hint_emission_rate"), 0.0),
            "emotional_intensity_distribution": soak.get("emotional_intensity_distribution", {}),
            "shift_bucket_distribution": soak.get("shift_bucket_distribution", {}),
            "mixed_affect_bucket_distribution": soak.get("mixed_affect_bucket_distribution", {}),
            "recovery_marker_bucket_distribution": soak.get("recovery_marker_bucket_distribution", {}),
            "support_need_bucket_distribution": soak.get("support_need_bucket_distribution", {}),
            "emotional_graph_low_confidence_suppressed_count": int(soak.get("emotional_graph_low_confidence_suppressed_count", 0)),
            "emotional_graph_safety_suppressed_count": int(soak.get("emotional_graph_safety_suppressed_count", 0)),
            "utility_emotional_suppression_count": int(soak.get("utility_emotional_suppression_count", 0)),
            "user_visible_emotion_label_block_count": int(soak.get("user_visible_emotion_label_block_count", 0)),
            "clinical_label_block_count": int(soak.get("clinical_label_block_count", 0)),
            "memory_write_block_count": int(soak.get("memory_write_block_count", 0)),
            "micro_bridge_presence_count": int(soak.get("micro_bridge_presence_count", 0)),
            "micro_bridge_neutral_count": int(soak.get("micro_bridge_neutral_count", 0)),
            "micro_bridge_context_injection_count": int(soak.get("micro_bridge_context_injection_count", 0)),
            "micro_bridge_low_confidence_suppressed_count": int(soak.get("micro_bridge_low_confidence_suppressed_count", 0)),
            "micro_intent_signal_count": int(soak.get("micro_intent_signal_count", 0)),
            "intent_clarity_distribution": soak.get("intent_clarity_distribution", {}),
            "request_type_distribution": soak.get("request_type_distribution", {}),
            "confusion_bucket_distribution": soak.get("confusion_bucket_distribution", {}),
            "trust_state_bucket_distribution": soak.get("trust_state_bucket_distribution", {}),
            "misunderstanding_risk_distribution": soak.get("misunderstanding_risk_distribution", {}),
            "repair_signal_count": int(soak.get("repair_signal_count", 0)),
            "correction_type_distribution": soak.get("correction_type_distribution", {}),
            "answer_success_distribution": soak.get("answer_success_distribution", {}),
            "frustration_bucket_distribution": soak.get("frustration_bucket_distribution", {}),
            "followup_pressure_risk_distribution": soak.get("followup_pressure_risk_distribution", {}),
            "micro_low_confidence_suppressed_count": int(soak.get("micro_low_confidence_suppressed_count", 0)),
            "micro_sensitive_suppressed_count": int(soak.get("micro_sensitive_suppressed_count", 0)),
            "micro_behavior_hint_emission_count": int(soak.get("micro_behavior_hint_emission_count", 0)),
            "micro_behavior_hint_suppressed_count": int(soak.get("micro_behavior_hint_suppressed_count", 0)),
            "correction_ack_hint_count": int(soak.get("correction_ack_hint_count", 0)),
            "clarification_hint_count": int(soak.get("clarification_hint_count", 0)),
            "reduce_questions_hint_count": int(soak.get("reduce_questions_hint_count", 0)),
            "directness_hint_count": int(soak.get("directness_hint_count", 0)),
            "avoid_lens_offer_hint_count": int(soak.get("avoid_lens_offer_hint_count", 0)),
            "truncation_repair_hint_count": int(soak.get("truncation_repair_hint_count", 0)),
            "low_confidence_behavior_suppressed_count": int(soak.get("low_confidence_behavior_suppressed_count", 0)),
            "safety_behavior_suppressed_count": int(soak.get("safety_behavior_suppressed_count", 0)),
            "response_length_no_increase_check": soak.get("response_length_no_increase_check", "ok"),
            "forced_question_no_increase_check": soak.get("forced_question_no_increase_check", "ok"),
            "therapeutic_drift_check": soak.get("therapeutic_drift_check", "ok"),
