...
def _lux_language_internal_hint_text(hint: dict[str, Any]) -> str:
    if not isinstance(hint, dict):
        return ""
    confidence = safe_float(hint.get("confidence", 0.0), 0.0)
    if confidence < 0.75:
        return ""
    summary = _lc(hint.get("safe_summary"))
    if summary in {
        ...
        return ""
    if bool(hint.get("should_avoid_identity_inference")):
        return "Do not infer identity, belief, origin, politics, class, or region from language choice; answer only the explicit request."
    if bool(hint.get("should_ask_clarification")):
        return "If the wording is too compressed to resolve safely, ask one short clarifying question instead of guessing."
    if bool(hint.get("should_silent_typo_repair")):
        return "If a minor typo is obvious, silently read the intended wording and do not correct or shame the user."
    if bool(hint.get("should_treat_as_irony")):
        return "If positive wording conflicts with a negative event, respond to the underlying frustration without naming it."
    if bool(hint.get("should_preserve_code_switching")):
        return "Preserve mixed-language terms where they carry the user's meaning; avoid mechanical translation."
    if bool(hint.get("should_avoid_literal_translation")):
        return "Avoid mechanical literal reading; answer the likely plain meaning."
    if bool(hint.get("should_avoid_dialect_imitation")):
        return "Use normal respectful language; do not imitate the user's wording style."
    return ""
...
class LearningContextBuilder:
    ...
    def build_live_context(
        ...
        lux_language_sense_bundle: dict[str, Any] | None = None,
    ) -> dict[str, Any]:
        _ = profile, session
        ...
        lux_language_sense_bundle = lux_language_sense_bundle or {}
        max_items = max(5, min(max_items, 12))
        now_utc = datetime.now(timezone.utc)
        lux_language_sense_active = bool(lux_language_sense_bundle.get("active", False))
        lux_language_sense_confidence = safe_float(lux_language_sense_bundle.get("confidence", 0.0), 0.0)
        lux_language_sense_meta = {
            "present": bool(lux_language_sense_bundle),
            "active": lux_language_sense_active,
            "context_injected": False,
            "low_confidence_suppressed": False,
            "confidence": round(clamp(lux_language_sense_confidence), 4),
            "safe_summary": _norm_text(
                lux_language_sense_bundle.get("safe_summary", "lux_language_sense_neutral_optional_hook")
            )[:80],
        }
        lux_language_hint = (
            lux_language_sense_bundle.get("language_intent_repair_hint", {})
            if isinstance(lux_language_sense_bundle.get("language_intent_repair_hint"), dict)
            else {}
        )
        lux_language_behavior_hint = _lux_language_internal_hint_text(lux_language_hint)
        if lux_language_sense_active and lux_language_behavior_hint:
            lux_language_sense_meta["context_injected"] = True
            lux_language_sense_meta["behavior_hint"] = lux_language_behavior_hint
            lux_language_sense_meta["hint_style"] = _norm_text(lux_language_hint.get("hint_style"))[:40]
        elif _lc(lux_language_hint.get("safe_summary")) == "language_intent_repair_hint_low_confidence_suppressed":
            lux_language_sense_meta["low_confidence_suppressed"] = True
