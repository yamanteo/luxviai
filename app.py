...
from luxcode_swarm.api import build_swarm_router
from luxcode_swarm.router import SwarmOrchestrator
from luxcode_swarm.schemas import SwarmConfig
from luxcode_agent_runtime import CODER_SYSTEM_PROMPT, run_agent, run_tool_json
...
from explorer_agent_preview import (
    build_explorer_agent_preview,
...
class CreditSaverPreviewRequest(BaseModel):
    behavior: Optional[str] = Field(default=None, max_length=80)
    symptom: str = Field(default="", max_length=2000)
    expected_result: str = Field(default="", max_length=2000)
    actual_result: str = Field(default="", max_length=2000)
...
class LuxCodeTaskCreateRequest(BaseModel):
    ...
    selectedFolder: Optional[str] = Field(default=None, max_length=1000)
    folderPath: Optional[str] = Field(default=None, max_length=1000)
    selectedFolders: List[str] = Field(default_factory=list)
    repository_root: Optional[str] = Field(default=None, max_length=600)
    repositoryRoot: Optional[str] = Field(default=None, max_length=1000)
    rootDir: Optional[str] = Field(default=None, max_length=1000)
    suspected_files: List[str] = Field(default_factory=list)
    changed_files: List[str] = Field(default_factory=list)
...
class LuxCodeAgentRunRequest(BaseModel):
...
app.include_router(build_swarm_router(BASE_DIR))
...
@app.get("/debug/credit-saver-status")
async def debug_credit_saver_status():
    return credit_saver_status()


@app.get("/debug/credit-saver-registry")
async def debug_credit_saver_registry():
    return credit_saver_registry()
...
@app.get("/debug/explorer-agent-status")
async def debug_explorer_agent_status():
    return explorer_agent_status()


@app.get("/debug/explorer-agent-registry")
async def debug_explorer_agent_registry():
    return explorer_agent_registry()


@app.post("/debug/explorer-agent-preview")
async def debug_explorer_agent_preview(payload: ExplorerAgentPreviewRequest):
    return build_explorer_agent_preview(
        command=payload.command,
        project_area=payload.project_area,
        related_layer=payload.related_layer,
    )
...
LEGACY_SWARM_TASKS: Dict[str, Dict[str, Any]] = {}
    ...
    if isinstance(value, list):
        for item in value:
            ...
            if text:
                return text
    if isinstance(value, dict):
        for key in ("path", "folder", "root", "workspace_root", "repository_root", "repositoryRoot"):
...
def _resolve_agent_workspace_root(value: Optional[str]) -> str:
...
def _resolve_legacy_swarm_workspace(payload: Dict[str, Any]) -> str:
    ...
    candidates = [
        ...
        payload.get("selectedFolders"),
        payload.get("selected_folders"),
        payload.get("repository_root"),
        payload.get("repositoryRoot"),
        payload.get("rootDir"),
    ]
...
def _legacy_swarm_task_id(result: Dict[str, Any]) -> str:
    return (
        ...
        or str((result.get("task") or {}).get("task_id") or "")
        or str((result.get("approval") or {}).get("task_id") or "")
    )
...
def _map_swarm_to_legacy_create_response(result: Dict[str, Any], workspace_root: str) -> Dict[str, Any]:
    ...
    if status == "awaiting_cost_approval":
        ...
        legacy["next_action"] = "Paid model approval required"
    if result.get("ok"):
        legacy["progress"] = "Swarm task completed"
        legacy["result"] = (result.get("task") or {}).get("result")
    return legacy


def _run_legacy_swarm_bridge(payload: Dict[str, Any]) -> Dict[str, Any]:
    workspace_root = _resolve_legacy_swarm_workspace(payload)
    if not workspace_root:
        return {
            "ok": False,
            "status": "failed",
            "error": "missing_selected_folder",
            "message": "missing_selected_folder",
            "blocked_reasons": ["missing_selected_folder"],
            "can_advance": False,
    ...
    prompt = str(payload.get("original_request") or payload.get("prompt") or payload.get("command") or "").strip()
    if not prompt:
        return {
            "ok": False,
            "status": "failed",
            ...
            "workspace_root": workspace_root,
            "blocked_reasons": ["missing_prompt"],
            "can_advance": False,
            "requires_user_approval": False,
        }
    logging.info("LUXCODE_SWARM_LEGACY_BRIDGE workspace_root=%s", workspace_root)
    result = SwarmOrchestrator(SwarmConfig(workspace_root=workspace_root)).execute(prompt)
    legacy = _map_swarm_to_legacy_create_response(result, workspace_root)
...
def _legacy_state_machine_create_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
    selected_folders: List[str] = []
    for key in ("selected_folders", "selectedFolders"):
        for item in _as_list(payload.get(key)):
            text = _first_text(item)
            if text and text not in selected_folders:
                selected_folders.append(text)
    for key in ("selectedFolder", "folderPath"):
        text = _first_text(payload.get(key))
        if text and text not in selected_folders:
            selected_folders.append(text)
    repository_root = (
        _first_text(payload.get("repository_root"))
        or _first_text(payload.get("repositoryRoot"))
        or _first_text(payload.get("workspace_root"))
        or _first_text(payload.get("rootDir"))
    )
    return {
        "original_request": str(payload.get("original_request") or payload.get("prompt") or payload.get("command") or ""),
        ...
        "requested_files": list(payload.get("requested_files") or []),
        "forbidden_files": list(payload.get("forbidden_files") or []),
        "permission_mode": str(payload.get("permission_mode") or "approval_required"),
        "scope_items": list(payload.get("scope_items") or []),
        "selected_folders": selected_folders,
        "autonomy_budgets": dict(payload.get("autonomy_budgets") or {}),
    }
...
async def luxcode_workspace_select_folder(payload: Dict[str, Any]):
...
@app.get("/luxcode-conversation/{session_id}/events")
async def luxcode_conversation_events_compat(session_id: str, accept: str = Header(default="")):
    if "text/event-stream" in accept.lower():
        return Response(
            status_code=204,
            headers={
                "Cache-Control": "no-store",
                "X-LuxCode-Legacy-Endpoint": "removed",
            },
        )
    return JSONResponse(
        status_code=410,
        content={
            ...
            "message": (
                "Bu endpoint kaldırıldı. LUXCODE UI, yeni task API'sini kullanır: "
                "/luxcode-task/create, /luxcode-task/{task_id}"
            ),
            "session_id": session_id,
            "replacement": {
                "create": "/luxcode-task/create",
                "status": "/luxcode-task/{task_id}",
                "advance": "/luxcode-task/advance",
                "approve": "/luxcode-task/approve",
                "pause": "/luxcode-task/pause",
                "resume": "/luxcode-task/resume",
                "cancel": "/luxcode-task/cancel",
            },
            "hint": "Tarayıcı önbelleğini temizleyip /luxcode-v1/ sayfasını Ctrl+F5 ile açın.",
        },
    )
...
@app.get("/luxcode-agent/system-prompt")
async def luxcode_agent_system_prompt_endpoint():
    return {
        "ok": True,
...
@app.post("/luxcode-agent/tool")
async def luxcode_agent_tool_endpoint(payload: LuxCodeAgentToolRequest):
    workspace_root = _resolve_agent_workspace_root(payload.workspace_root)
    ok, output = run_tool_json({"tool": payload.tool, "params": payload.params}, workspace_root=workspace_root)
    return {"ok": ok, **output}
...
@app.get("/debug/live-readiness")
async def debug_live_readiness():
...
<body>
  <main>
    ...
    <div class="bar">
      <button data-explorer-agent-command="stop continue arm typewriter iliskilerini kesfet" data-explorer-agent-area="stop_continue">Explorer Agent: Stop/Continue</button>
  ...
  <script>
    ...
    async function loadExplorerAgentPreview(projectArea, command) {
      statusEl.textContent = "Loading explorer agent preview";
      output.textContent = "{}";
      try {
        const response = await fetch("/debug/explorer-agent-preview", {
          method: "POST",
          headers: { "Accept": "application/json", "Content-Type": "application/json" },
          body: JSON.stringify({
            command,
        ...
        const data = await response.json();
        statusEl.textContent = response.ok ? "Loaded explorer agent preview" : "Request failed: " + response.status;
        output.textContent = JSON.stringify(data, null, 2);
      } catch (err) {
        statusEl.textContent = "Request error";
        output.textContent = String(err);
    ...
    document.querySelectorAll("button[data-explorer-agent-command]").forEach((button) => {
